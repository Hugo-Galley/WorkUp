...
def callJoobleApi(conn,cursor,keyword, location):
    ...
    response = requests.post(configData["Api"]["Jooble"]["Host"] + configData["Api"]["Jooble"]["Key"], headers=headers, data=json.dumps(params))
            ...
            try:
                cursor.execute(
                    ...
                    (
                        ...
                        data["results"][i]["company"]["display_name"] if "display_name" in data["results"][i]["company"] else "Unknown",
                        data["results"][i]["salary_is_predicted"],
                        data["results"][i]["redirect_url"],
                        data["results"][i]["location"]["display_name"] if "display_name" in data["results"][i]["location"] else "Unknown",
...
def callLaBonneAlteranceApi(conn,cursor,insee,diploma):
    ...
    response = requests.get(configData["Api"]["LaBonneAlternance"]["Host"], params=params)
    if response.status_code == 200:
        data = response.json()
            ...
            try:
                cursor.execute(
                    ...
                    (
                        ...
                        clean_text(data["jobs"]["partnerJobs"]["results"][i]["job"]["description"]),
